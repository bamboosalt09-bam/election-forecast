...
from election_forecast.v28_runtime import ensure_v28_runtime


try:
    PACKAGE_VERSION = version("election-forecast")
except PackageNotFoundError:  # source-tree execution before installation
    PACKAGE_VERSION = "0.28.0.dev0"
ACTIVE_MODEL_VERSION = "V28"
...
def _repository_script(name: str) -> Path:
    ...
    if source_path.is_file():
        return source_path
    runtime_path = ensure_v28_runtime() / "scripts" / name
    if not runtime_path.is_file():
        raise SystemExit(f"the verified V28 runtime does not contain {name}")
    return runtime_path


def _v28_root(script: Path) -> Path:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Korean presidential election forecast engine (active frozen model V28)"
    )
    ...
    run_current = subparsers.add_parser(
        ...
        help="Reproduce the active frozen V28 historical model",
    )
    run_current.add_argument("--output-dir")

    subparsers.add_parser(
        "audit-current-presidential",
        help="Audit the active V28 artifact and V23-V27 rollback boundaries",
    )

    subparsers.add_parser(
        "verify-current-presidential",
        help="Rebuild V28 in isolation and compare it with the frozen artifact",
    )
...
def main() -> None:
    ...
    if args.command == "run-current-presidential":
        ...
        runtime_root = _v28_root(script)
        output_dir = (
            Path(args.output_dir).expanduser().resolve()
            if args.output_dir
            else (Path.cwd() / "outputs" / "reproduction_v28").resolve()
        )
    ...
    if args.command == "audit-current-presidential":
        script = _repository_script("audit_public_active_presidential_model_v28.py")
        command = [sys.executable, str(script)]
        raise SystemExit(subprocess.run(command, cwd=_v28_root(script), check=False).returncode)

    if args.command == "verify-current-presidential":
        script = _repository_script("verify_v28_clean_reproduction.py")
        command = [sys.executable, str(script)]
        raise SystemExit(subprocess.run(command, cwd=_v28_root(script), check=False).returncode)
