"""Select an untouched V23-S supplement from the broad 25,000-row corpus."""
...
SEED = "stance-v23s-broad-holdout-5000-v1"
...
def main() -> None:
    ...
    parser.add_argument("--used", type=Path, action="append", required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--frozen-v23s-sha256", required=True)
    args = parser.parse_args()

    source = args.input.resolve()
    used_paths = [path.resolve() for path in args.used]
    destination = args.output.resolve()
    if destination.exists() or destination.with_suffix(".state.json").exists():
    ...
    for path in used_paths:
        used.update(
            pd.read_csv(path, encoding="utf-8-sig", usecols=["text_sha256"])[
                "text_sha256"
            ].astype(str)
        )
    ...
    if len(output) != 5_000 or output["text_sha256"].duplicated().any():
        raise RuntimeError("V23-S holdout supplement is not 5,000 unique rows")
    if output["text_sha256"].astype(str).isin(used).any():
        raise RuntimeError("V23-S holdout overlaps a prior development or audit corpus")

    destination.parent.mkdir(parents=True, exist_ok=True)
    output.to_csv(destination, index=False, encoding="utf-8-sig")
    state = {
        "status": "v23s_broad_holdout_supplement_complete",
        "rows": len(output),
        "seed": SEED,
        ...
        "excluded_source_sha256": [_sha256(path) for path in used_paths],
        "excluded_unique_hashes": len(used),
