...
FROZEN = ROOT / "outputs/prospective_pres_2025_v31"
...
def main() -> None:
    ...
    if absent:
        print("[V31 prospective reproduction: SKIPPED - required input unresolved]")
    ...
    original = v31.OUTPUT_DIR
    with tempfile.TemporaryDirectory(prefix="election_forecast_v31_prospective_") as temporary:
        destination = Path(temporary) / "prospective_pres_2025_v31"
        try:
            v31.OUTPUT_DIR = destination
            v31.run()
        finally:
            v31.OUTPUT_DIR = original
    ...
    print("[V31 prospective reproduction: PASS]")
