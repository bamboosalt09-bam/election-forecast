...
hash-indexed V32 runtime embedded in the wheel.
"""
...
def _audit_runtime(payload: bytes) -> int:
    with ZipFile(io.BytesIO(payload)) as runtime:
        ...
        if manifest.get("active_version") != "v32":
            raise RuntimeError("wheel runtime does not declare active V32")
        ...
        if manifest.get("frozen_prediction_sha256") != FROZEN_V32_SHA256:
            raise RuntimeError("wheel runtime frozen V32 hash declaration drifted")
        ...
        if hashlib.sha256(frozen).hexdigest() != FROZEN_V32_SHA256:
            raise RuntimeError("embedded frozen V32 prediction hash drifted")
        return len(indexed)
...
def audit_wheel(path: Path) -> tuple[int, int]:
    with ZipFile(path) as wheel:
        ...
        for name in names:
            _assert_public(name)
        runtime_names = [
            name for name in names if name.endswith("election_forecast/_v32_runtime.zip")
        ]
        if len(runtime_names) != 1:
            raise RuntimeError(
                f"wheel must contain exactly one V32 runtime archive: {path.name}"
            )
...
def main() -> None:
    ...
    print(f"frozen_v32_sha256={FROZEN_V32_SHA256}")
