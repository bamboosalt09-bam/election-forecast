"""Build chronological predictive intervals for V32."""
...
INPUT = ROOT / "outputs/active_presidential_nested_v32/nested_predictions.csv"
OUTPUT_DIR = ROOT / "outputs/active_presidential_nested_v32"
...
def build(*, n_sim: int = 50_000, seed: int = 30_820, output_dir: Path = OUTPUT_DIR):
    ...
    payload["schema"] = "active_v32_national_predictive_intervals_v1"
    payload["model_version"] = "v32"
    payload["development_outcome_warning"] = (
        "V32's scored predictions are byte-identical to V31's, so these "
        "intervals are V31's intervals: the same input produces the same "
        "residual scale. Nothing about them is re-evidenced here, and the "
        "chain they were inherited through reaches back to V28. Coverage "
