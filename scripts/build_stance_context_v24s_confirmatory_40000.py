"""Build a fresh 40,000-row confirmatory corpus after V24-S is frozen."""
...
SEED = "stance-context-v24s-confirmatory-40000-v1"
...
def main() -> None:
    ...
    parser.add_argument("--frozen-v24s-sha256", required=True)
    args = parser.parse_args()
    source = args.input.resolve()
    output_dir = args.output_dir.resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    selected_path = output_dir / "selected_rows.csv"
    output_path = output_dir / "stance_context_v24s_confirmatory_40000.csv"
    ...
    print(f"[V24-S confirmatory selection] excluded hashes: {len(excluded):,}", flush=True)
    ...
    state = {
        "status": "v24s_confirmatory_corpus_complete",
        "rows": len(output),
        "seed": SEED,
        "frozen_v24s_sha256": args.frozen_v24s_sha256,
