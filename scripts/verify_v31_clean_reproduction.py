"""Rebuild V31 separately and compare it with the frozen active artifact."""
...
FROZEN = ROOT / "outputs/active_presidential_nested_v31/nested_predictions.csv"


def main() -> None:
    with tempfile.TemporaryDirectory(prefix="election_forecast_v31_") as temporary:
        destination = Path(temporary) / "active_presidential_nested_v31"
        v31.run(destination)
        ...
        if paths.str.contains("assembly_issue_character_overlay", regex=False).any():
            raise RuntimeError("clean V31 reproduction retained sentence-level overlay")
        retained = paths.str.endswith(
            "data/raw/auto_issue_seed/candidate_issue_profile.csv"
        )
        if int(retained.sum()) != 1:
            raise RuntimeError("clean V31 reproduction lost its disclosed frozen profile")
        audit = pd.read_csv(
            destination / "multiplicative_dispersion_expansion_audit.csv",
            encoding="utf-8-sig",
        )
        if not bool((audit["max_candidate_level_shift_pp"].abs() < 1e-9).all()):
            raise RuntimeError("clean V31 reproduction moved a candidate national level")
    print("[clean V31 reproduction: PASS]")
