"""Run the outcome-free 2025 demonstration through the promoted V31 layer."""
...
OUTPUT_DIR = ROOT / "outputs" / "prospective_pres_2025_v31"
...
WEIGHT_COLUMN = "v31_prior_election_vote_weight"
...
def _expand_in_place() -> None:
    ...
    stage["v31_pre_multiplicative_expansion_pred"] = stage["layer_pred"]
    stage, audit = multiplicative_dispersion_expansion.apply_multiplicative_dispersion_expansion(
    ...
    audit.to_csv(
        OUTPUT_DIR / "multiplicative_dispersion_expansion_audit.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    added = [
        ROOT / "presidential_issue_engine/multiplicative_dispersion_expansion.py",
        ROOT / "scripts/run_prospective_forecast_v31.py",
        ROOT / "scripts/run_active_presidential_model_v31.py",
    ]
    manifest = (
        pd.concat(
            [
    ...
    payload["v31_multiplicative_dispersion_expansion"] = {
