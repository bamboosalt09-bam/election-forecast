"""Inventory manual controls after promotion of active v17."""
...
OUTPUT_DIR = ROOT / "outputs" / "automation_status_v4"
...
INPUT_STATUS = [
    ...
    ("third_candidate_profile", "manual_active", "speech-only v12 regressed in 2017; rebuild from prior election stature"),
    ("third_candidate_pressure", "manual_active", "automatic source-lane pressure v12 regressed in 2017"),
    ("candidate_regional_base", "automatic_active", "footprint-controlled official history v9"),
    ("chungcheong_identity", "mixed_manual_active", "full-election reservoir and 2007/2017 routing automatic; 2002/2012 event routing curated"),
    ("mega_issue_intensity", "manual_active", "automatic transcript intensity not yet promoted"),
    ("mega_issue_taxonomy", "manual_active", "needs conservative pre-election event classifier"),
...
def main() -> None:
    ...
    summary = {
        ...
        "automatic_active_inputs": int(inputs["automation_status"].eq("automatic_active").sum()),
        "manual_active_inputs": int(inputs["automation_status"].eq("manual_active").sum()),
        "mixed_manual_active_inputs": int(inputs["automation_status"].eq("mixed_manual_active").sum()),
        "mostly_automatic_inputs": int(inputs["automation_status"].eq("mostly_automatic_active").sum()),
