"""Apply frozen V16 attribution gates to a fresh NLI prediction corpus."""
...
from election_forecast.stance_context_v16 import (  # noqa: E402
...
def main() -> None:
    ...
    output_path = output_dir / "context_predictions_v16.csv"
    if output_path.exists():
        raise FileExistsError(f"V16 prediction output already exists: {output_path}")
    frame = pd.read_csv(source, encoding="utf-8-sig").fillna("")
    validate_shadow_corpus(frame)
    prediction, reasons = apply_contextual_attribution_gate_v16(
        frame.to_dict(orient="records"), frame["context_prediction"]
    )
    output = frame.copy()
    output["v16_prediction"] = prediction
    output["v16_abstention_reasons"] = reasons
    output.to_csv(output_path, index=False, encoding="utf-8-sig")
    state = {
        "status": "frozen_v16_shadow_application_complete",
        ...
        "prediction_counts": output["v16_prediction"].value_counts().to_dict(),
