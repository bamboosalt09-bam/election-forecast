...
위 표는 **V31의 2025 예보를 채점한 것이며 활성 V32의 것이 아닙니다.** V32는 죽어 있던 다섯 피처 계열을 되살리면서 2025 예보를 지역 최대 `1.2381%p`, 전국 `0.4152%p` 움직였고, 그 예보에 대한 사후 채점은 **아직 수행하지 않았습니다.** 사후 채점은 공시된 read-only 예외이지만 버전 선택에 쓰이지 않는다는 조건 아래에서만 의미가 있고, 승격이 끝난 지금 굳이 새로 계산하지 않은 채 남겨 둡니다. 표를 V32로 고쳐 붙이는 것은 만들지도 않은 예보에 숫자를 귀속시키는 일이라 하지 않았습니다.
