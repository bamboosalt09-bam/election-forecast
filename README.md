...
이 표는 **V27 시점의 측정이며 이후 재측정하지 않았습니다.** 2025 실제 결과는 적합·선택·조정은 물론 **오차 계산에도 쓰지 않는다**는 것이 이 프로젝트의 서약이고, 그 예외로 한 번 공개한 기록이 이것입니다. V31로 다시 재려면 같은 예외를 한 번 더 여는 셈이라 하지 않았습니다. 전국 예측 세 값은 V27 이후 변하지 않았고(경계 강제로 0.0065%p 이동), 지역 수치는 그동안 바뀌었으므로 위 지역 MAE 두 줄은 현재 모델의 값이 아닙니다.
