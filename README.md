...
이 러너는 V26 실행 계보, V27에서 계승한 지역 양극화층, V28의 외부모델 경계, 그리고 V31의 제3후보 분산 확대층을 거쳐 2025-06-02(D-1)까지 공개된 후보 명부·국회 발언 맥락만 사용합니다. 외부모델 파생 overlay와 2025 실제 결과를 읽지 않고 성능지표도 계산하지 않습니다. 보존 가중치는 2022 지역 투표량이며 입력 SHA-256과 결과정보 미사용 선언을 `run_manifest.json`에 기록합니다.
