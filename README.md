...
거대 이슈 강도는 `{0.50, 0.75, 1.00, 2.00}` 네 값만 도달 가능하고 활성화는 `(강도−1)`을 0~1로 자르므로, 직접 충격은 **완전 무력이거나 완전 포화** 둘 중 하나입니다. 그 사이를 분류기 자신의 게이트 비율로 채우는 후보를 2×2로 측정했습니다.
...
| 정렬 단독 | 3.4403 | 0.9896 |
| 사다리+정렬 | **3.4214** | **0.7210** |

정렬 단독은 기준선과 자릿수까지 동일하고 사다리 단독은 파국이며, 둘이 함께일 때만 개선됩니다. 새 상수는 없고 2017은 구조적으로 보존됩니다. 그럼에도 **승격하지 않았습니다** — 결합된 두 변경이고, 조합을 패널 지표 비교로 골랐으며, 패널 최악인 2002가 전혀 움직이지 않기 때문입니다. 전체 근거는 [V25 강도 사다리 실험](docs/EXPERIMENT_V25_INTENSITY_LADDER_20260822.md)에 있습니다.
