...
def test_public_cli_reports_v29(capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("sys.argv", ["election-forecast", "show-active-version"])
    cli.main()
    assert capsys.readouterr().out.strip() == "V29"
...
def test_public_cli_exposes_current_workflows() -> None:
    ...
    assert "active frozen model V29" in help_text


def test_package_and_model_versions_are_explicit() -> None:
    assert cli.PACKAGE_VERSION == "0.29.0.dev0"
    assert cli.ACTIVE_MODEL_VERSION == "V29"
