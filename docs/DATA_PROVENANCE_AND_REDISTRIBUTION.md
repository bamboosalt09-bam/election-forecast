...
| Historical National Assembly members and proceedings | derived salience/link tables and historical source hashes | National Assembly Library, National Academic Information Cloud legislative datasets | The source XLSX/ZIP files were downloaded from the official legislative-dataset catalogue. V28 distributes only compact project-derived aggregates with attribution; source workbooks, full transcripts and sentence corpora are excluded. The provider terms require attribution and prohibit selling or sharing the supplied information unchanged without a service layer. |
...
| Market series | `data/raw/official_sources/bok_ecos_kospi_manifest.json` | Bank of Korea ECOS / Korea Exchange | Only collection code and provenance metadata are public. The local row-level KOSPI history and ECOS cache are Git-ignored and excluded from the wheel. V28's clean reproduction is verified without them. |
...
| Model outputs and audits | `outputs/active_presidential_nested_v23` through `v28`, prospective manifests, evaluation extracts | Project calculations over the above inputs | Project-authored calculations may be reused with attribution, but embedded names, factual records and any source-derived excerpts remain subject to their source terms. Frozen versions must not be modified in place. V28 retains one disclosed compact external-model-derived candidate-issue aggregate; model weights, source sentences and the direct overlay are excluded. |
