...
| Historical National Assembly members and proceedings | derived salience/link tables and historical source hashes | National Assembly Library, National Academic Information Cloud legislative datasets | The source XLSX/ZIP files were downloaded from the official legislative-dataset catalogue. V30 distributes only compact project-derived aggregates with attribution; source workbooks, full transcripts and sentence corpora are excluded. The provider terms require attribution and prohibit selling or sharing the supplied information unchanged without a service layer. |
...
| Historical presidential turnout (1997) | `presidential_issue_engine/fixed_dataset/pres_1997_regional_turnout.csv` | 국사편찬위원회 한국사데이터베이스, republishing National Election Commission returns for the 15th presidential election | Included as a 16-row factual extract - electorate, votes cast and valid votes per region - with attribution and no narrative text. It is the forecast-time weight V30 uses for 2002 and is registered separately rather than under project authorship, because the numbers are an official record this project transcribed rather than derived. |
...
| Model outputs and audits | `outputs/active_presidential_nested_v23` through `v30`, prospective manifests, evaluation extracts | Project calculations over the above inputs | Project-authored calculations may be reused with attribution, but embedded names, factual records and any source-derived excerpts remain subject to their source terms. Frozen versions must not be modified in place. V30 retains one disclosed compact external-model-derived candidate-issue aggregate; model weights, source sentences and the direct overlay are excluded. |
