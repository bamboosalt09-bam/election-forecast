...
small canonical inputs, and the frozen artifacts needed to audit active V23.
...
V23 is frozen. Experiments must use a new versioned output directory and must
not overwrite V23 code, configuration, predictions, or manifests. Promotion of
a later version requires all of the following in one pull request:

1. a new versioned configuration and runner;
...
Git tracks small canonical inputs and the frozen V23 audit bundle. It does not
...
suite, and the frozen V23 audit. Full Assembly reprocessing and large NLP
